(function() {
	...
	// in the Clear Step Table. POST method is allowed          //
	...
	var gvTableName = 'COV_VAT_CLEAR_STEP';
		...
		try {
			...
			while (oResultSet.next()) {
				...
				var record = {
					...
					STRUCTURE: oResultSet.getString(2),
					FIELD: oResultSet.getString(3),
					ENTIRE_FIELD: oResultSet.getString(4),
					PART_FIELD: oResultSet.getString(5),
					FROM_CHARACTER: oResultSet.getString(6),
					NUMBER_OF_CHARACTERS: oResultSet.getString(7),
					CONDITION_ID: oResultSet.getString(8)
				};
	...
	function _createEntry() {
		try {
			...
			if (oBody.ENTIRE_FIELD) {
				oStatement.setString(4, oBody.ENTIRE_FIELD.toString());
			} else {
				oStatement.setInt(4, 0);
			}
			//Part Field Flag
			if (oBody.PART_FIELD) {
				oStatement.setString(5, oBody.PART_FIELD.toString());
			} else {
				oStatement.setInt(5, 0);
			}
			//From Character
			if (oBody.FROM_CHARACTER) {
				oStatement.setInt(6, parseFloat(oBody.FROM_CHARACTER));
			} else {
				oStatement.setInt(6, 0);
			}
			//Number of Characters
			if (oBody.NUMBER_OF_CHARACTERS) {
				oStatement.setInt(7, parseFloat(oBody.NUMBER_OF_CHARACTERS));
			} else {
				oStatement.setInt(7, 0);
			}
			//Condition Id
			if (oBody.CONDITION_ID) {
				oStatement.setInt(8, parseFloat(oBody.CONDITION_ID));
			} else {
	...
	function _updateEntry() {
		try {
			...
			var oStatement = oConnection.prepareStatement(
				...
				"\" SET STRUCTURE = ?, FIELD = ?, ENTIRE_FIELD = ?, PART_FIELD = ?, FROM_CHARACTER = ?, NUMBER_OF_CHARACTERS = ?, CONDITION_ID = ? WHERE STEP_ID = ?"
			);
			...
			//Entire Field Flag
			if (oBody.ENTIRE_FIELD) {
				oStatement.setString(3, oBody.ENTIRE_FIELD.toString());
			} else {
				oStatement.setInt(3, 0);
			}
			//Part Field Flag
			if (oBody.PART_FIELD) {
				oStatement.setString(4, oBody.PART_FIELD.toString());
			} else {
				oStatement.setInt(4, 0);
			}
			//From Character
			if (oBody.FROM_CHARACTER) {
				oStatement.setInt(5, parseFloat(oBody.FROM_CHARACTER));
			} else {
				oStatement.setInt(5, 0);
			}
			//Number of Characters
			if (oBody.NUMBER_OF_CHARACTERS) {
				oStatement.setInt(6, parseFloat(oBody.NUMBER_OF_CHARACTERS));
			} else {
				oStatement.setInt(6, 0);
			}
			//Condition Id
			if (oBody.CONDITION_ID) {
				oStatement.setInt(7, parseFloat(oBody.CONDITION_ID));
			} else {
				oStatement.setInt(7, 0);
			}

			//Step Id
			oStatement.setInt(8, parseFloat(oBody.STEP_ID));
